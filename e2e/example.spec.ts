...
test.describe('Solana Devnet dApp', () => {
  test('should load the application and show wallet connection', async ({ page }) => {
    ...
    // Check that the wallet connect button is present (flexible: match "Connect Wallet" or "Select Wallet")
    const connectButton = await page.locator('button', { hasText: /connect|select wallet/i }).first()
  ...
  test('should show devnet warning when not connected', async ({ page }) => {
    await page.goto('/')
    // Check that the airdrop panel shows devnet-only message (actual text)
  ...
  test('should have proper form validation in transfer form', async ({ page }) => {
    ...
    // Check that all form fields are present (labels match app)
    ...
    await expect(transferForm.getByRole('button', { name: /transfer token/i })).toBeDisabled()
  })

  test('should display proper footer information', async ({ page }) => {
    await page.goto('/')
    // Check footer content (actual text)
    await expect(page.getByText('Built by DOLOS+ with Solana Web3.js')).toBeVisible()
    await expect(page.getByText('This dApp is configured for Solana devnet. Make sure your wallet is set to devnet.')).toBeVisible()
  })
})
